...
GXWindow {
  ...
  property bool boFullPanel
  property bool boFirstTime: true
  property bool boReady: mCXStatus.mDaemonStatus === CXDefinitions.EServiceReady || !boFullPanel ? true : false
  property bool boClosing: mCXStatus.mDaemonStatus === CXDefinitions.EServiceClosing && boFullPanel ? true : false
  ...
  OpacityMask {
    ...
    Component.onCompleted: {
      ...
      var vaX = 0
      if((mCXDefinitions.mX < 0) || !boFullPanel) vaX = (Screen.width - wiRoot.width) / 2
      else vaX = mCXDefinitions.mX
      wiRoot.x = vaX
      var vaY
      if((mCXDefinitions.mY < 0) || !boFullPanel) vaY = (Screen.height - wiRoot.height) / 2
    ...
    AXFrame {
      id: rcNameTitle
      color: (boClosing && boFullPanel) ? coCriticalBackgroundColor : coInfoBackgroundColor
      anchors.centerIn: rcCover
      opacity: (!boReady && boFullPanel) ? 1 : 0
      ...
      AXText {
        ...
        text: (boClosing && boFullPanel) ? qsTr("A critical update has been released. In order to preserve the security of the network as a whole an update is needed.\nWBC will close in a few seconds and the wizard will dowload necesary files") :
        ...
        color: (boClosing && boFullPanel) ? coCriticalTextColor : coInfoTextColor
